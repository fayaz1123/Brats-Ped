...
def train_all(
    ...
    if path_best.exists():
        state = torch.load(path_best, map_location=device, weights_only=False)
        ...
        logger.info(
            f"Loaded starting weights from {path_best}  (epoch={state.get('epoch', '?')}  "
            f"loss={best_loss:.4f})  -> continuing at epoch {start_epoch}"
        )
    else:
        logger.info(f"No checkpoint found at {path_best} — training from scratch.")
